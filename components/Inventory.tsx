  ...
  const handleAddProduct = (e: React.FormEvent) => {
    ...
    const newProduct: Product = {
      ...
      supplier: {
        ...
        leadTime: formData.supplierLeadTime || '7 days',
        qualityScore: 5,
        onTimeRate: 100
      },
      ...
      <div className="flex gap-0.5">
        {[1, 2, 3, 4, 5].map((s) => (
          <Star 
            key={s} 
            size={12} 
            className={s <= score ? "fill-amber-400 text-amber-400" : "fill-slate-200 text-slate-200"} 
          />
        ))}
      </div>
    );
          ...
          <div className="bg-white w-full max-w-4xl max-h-[90vh] rounded-[2.5rem] shadow-2xl overflow-hidden flex flex-col animate-in zoom-in duration-300">
            ...
            <div className="flex-1 overflow-y-auto p-8 pt-6 space-y-8">
              <div className="space-y-4">
                <div className="relative w-full h-[320px] rounded-[2rem] overflow-hidden border border-slate-100 shadow-sm bg-slate-50 group">
              ...
              <div className="grid grid-cols-1 lg:grid-cols-2 gap-8">
                <div className="space-y-4">
                  <h4 className="text-lg font-bold text-slate-800 flex items-center gap-2"><Tag className="text-indigo-600" size={20} /> Supplier Performance</h4>
                  {selectedProduct.supplier ? (
                    <div className="bg-white border border-slate-200 rounded-3xl p-8 space-y-6">
                      <div className="flex items-start justify-between">
                        <div>
                          <p className="text-2xl font-black text-slate-800 mb-1">{selectedProduct.supplier.name}</p>
                          <div className="flex items-center gap-2">
                            {renderStars(selectedProduct.supplier.qualityScore)}
                            <span className="text-xs font-bold text-slate-400 uppercase">({selectedProduct.supplier.qualityScore?.toFixed(1) || '0.0'})</span>
                          </div>
                        </div>
                      ...
                      <div className="grid grid-cols-2 gap-6">
                        <div className="space-y-1">
                          <p className="text-[10px] font-black text-slate-400 uppercase tracking-widest flex items-center gap-1.5"><Clock size={12} /> Lead Time</p>
                          <p className="text-lg font-bold text-slate-800">{selectedProduct.supplier.leadTime}</p>
                        </div>
                        <div className="space-y-1">
                          <p className="text-[10px] font-black text-slate-400 uppercase tracking-widest flex items-center gap-1.5"><Award size={12} /> On-Time Rate</p>
                          <div className="flex items-end gap-2">
                             <p className="text-lg font-bold text-emerald-600">{selectedProduct.supplier.onTimeRate || 100}%</p>
                             <div className="mb-1 flex-1 h-1.5 bg-slate-100 rounded-full overflow-hidden">
                                <div className="h-full bg-emerald-500 rounded-full" style={{ width: `${selectedProduct.supplier.onTimeRate || 100}%` }}></div>
                             </div>
                          </div>
                        </div>
                      </div>
                      ...
                      <div className="pt-4 border-t border-slate-50 space-y-3">
                        <div className="flex items-center gap-3 text-slate-600"><Phone size={16} className="text-slate-400" /><span className="text-sm font-medium">{selectedProduct.supplier.contact}</span></div>
                        <div className="flex items-center gap-3 text-slate-600"><Mail size={16} className="text-slate-400" /><span className="text-sm font-medium">{selectedProduct.supplier.email}</span></div>
                      </div>
                    </div>
                  ) : (<div className="bg-slate-50 rounded-3xl p-8 text-center border border-dashed border-slate-200 text-slate-400 italic">No supplier data.</div>)}
                </div>

                <div className="space-y-4">
                  <h4 className="text-lg font-bold text-slate-800 flex items-center gap-2"><History className="text-indigo-600" size={20} /> Stock Activity</h4>
                    ...
                    <div className="space-y-6 max-h-[300px] overflow-y-auto pr-2 scrollbar-hide">
                      {selectedProduct.stockHistory?.slice().reverse().map((log, i) => (
                        <div key={i} className="flex items-start gap-4 group">
                          <div className={`p-2 rounded-xl mt-1 transition-colors ${log.change > 0 ? 'bg-emerald-50 text-emerald-600 group-hover:bg-emerald-100' : 'bg-rose-50 text-rose-600 group-hover:bg-rose-100'}`}>
                            {log.change > 0 ? <ArrowUpRight size={16} /> : <ArrowDownRight size={16} />}
                          </div>
                          <div className="flex-1">
                            <div className="flex justify-between items-center mb-1">
                              <p className="text-sm font-bold text-slate-800 capitalize">
                                {log.type === 'adjustment' ? 'Stock Adjustment' : (log.type === 'restock' ? 'Restock Event' : 'Order Fulfillment')}
                              </p>
                              <p className="text-[10px] text-slate-400 font-bold uppercase">{new Date(log.timestamp).toLocaleDateString()}</p>
                            </div>
                            <p className="text-xs text-slate-500">{log.note || `Movement of ${Math.abs(log.change)} ${selectedProduct.unit}`}</p>
                          </div>
                        </div>
                      ))}
                    </div>
            ...
            <div className="p-8 bg-slate-50 border-t border-slate-100 flex justify-end gap-3">
              <button onClick={() => setIsAdjusting(selectedProduct)} className="px-6 py-3 bg-white border border-slate-200 text-slate-700 rounded-2xl font-bold hover:bg-slate-50 transition-colors flex items-center gap-2"><SlidersHorizontal size={18} /> Adjust</button>
              <button onClick={() => { setIsAdjusting(selectedProduct); setAdjustForm(prev => ({ ...prev, type: 'add', reason: 'Restock Shipment' })); }} className="px-6 py-3 bg-indigo-600 text-white rounded-2xl font-bold hover:bg-indigo-700 shadow-xl flex items-center gap-2"><Plus size={18} /> New Shipment</button>
            </div>
          </div>
        </div>
      )}
